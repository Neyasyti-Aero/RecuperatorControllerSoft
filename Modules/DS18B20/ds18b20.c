...
/*----------------------------------------------------------------------------*/
DS18B20_Status DS18B20_WriteScratchpad(DS18B20 *sensor, int8_t *data)
{
  DS18B20_Status result;
...
{
  DS18B20_Status result;
  uint8_t rxData[DS18B20_ROM_LEN_BYTES];
  ...
  for (uint8_t i = 0; i < DS18B20_ROM_LEN_BYTES; i++)
  {
    sensor->serialNumber[i] = rxData[i];
  }
...
DS18B20_Status DS18B20_MatchRom(DS18B20 *sensor)
{
  ...
  result = ExecuteCommand(sensor, matchRom, sensor->serialNumber);
...
/*----------------------------------------------------------------------------*/
void DS18B20_SetRom(DS18B20 *sensor, uint8_t* serialNumber)
{
	for (int i = 0; i < DS18B20_ROM_LEN_BYTES; i++)
		sensor->serialNumber[i] = serialNumber[i];
}
